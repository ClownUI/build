...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/clown/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/clown/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function lunch()
{
    ...
    if (echo -n $1 | grep -q -e "^clown_") ; then
      CLOWN_BUILD=$(echo -n $product | sed -e 's/^clown_//g')
    else
      CLOWN_BUILD=
    fi
    export CLOWN_BUILD
...
function _lunch_meat()
{
    ...
    if [[ -z "${ANDROID_QUIET_BUILD}" && -z "${CLOWN_BUILD}" ]]; then
